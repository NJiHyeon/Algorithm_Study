...
def solution(board):
    n = len(board)
    # 인덱싱의 편의를 위해 원본 배열에 상하좌우로 한 칸씩 늘린 새 배열 만든다. 
    ...
    while q:
        ...
        # (기본) 현재 상태에서 이동 가능한 상태를 구한다. 
        for next_pos in get_next_pos(cur_pos, new_board):
            # (기본) 상하좌우 이동이 가능한 경우를 구한다. 
            # 가로방향일 경우 회전 가능한 경우를 구한다. 
            # 세로방향일 경우 회전 가능한 경우를 구한다. 
